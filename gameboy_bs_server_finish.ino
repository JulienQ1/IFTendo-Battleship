...
void selectShip() {
    ...
    do {
        ...
        if (serverShips[selectedShipIndex].isAlive) {
            Ship& selectedShip = serverShips[selectedShipIndex];
    ...
    if (selectionCount > aliveShips) {
        ...
        isServerTurn = false;
        client.print("isServerTurn=false");
...
void moveShipToCursor() {
    if (selectedShipIndex != -1 && isWithinSurroundingArea(cursorX, cursorY, serverShips[selectedShipIndex])) {
        Ship& ship = serverShips[selectedShipIndex];
        ship.x = cursorX - ship.size / 2; // Center the ship on the cursor
        ship.y = cursorY;
        serverShips[selectedShipIndex].hasMoved = true;
...
void fireAtPosition() {
    if (isWithinSurroundingArea(cursorX, cursorY, serverShips[selectedShipIndex])) {
        // Local check for a hit
        bool hit = false;
        serverShips[selectedShipIndex].hasFired = true;
        for (int i = 0; i < 3; i++) {
            if (isShipHit(cursorX, cursorY, clientShips[i])) {
                clientShips[i].isAlive = false; // Mark the ship as sunk
                hit = true;
                break;
            }
        }
...
void processSinkMessage(const String& message) {
    ...
    for (int i = 0; i < 3; i++) {
        Ship& ship = serverShips[i]; // Replace with clientShips on the client side
...
void processMoveShipMessage(const String& message) {
    ...
    if (shipIndex >= 0 && shipIndex < 3) { // Assuming there are 3 ships
        clientShips[shipIndex].x = newX;
        clientShips[shipIndex].y = newY;
    }
}
...
void drawGrid() {
  ...
  // Draw sclient's ships (for the client side)
    for (int i = 0; i < 3; ++i) drawShip(serverShips[i]);

    // Draw server's ships (for the server side, if they are sunk)
    for (int i = 0; i < 3; ++i) {
        if (!clientShips[i].isAlive) {
            drawShip(clientShips[i]);
        }
    }

    if (selectedShipIndex != -1) {
        // If a ship is selected, mark its surrounding area
        markSurroundingArea(serverShips[selectedShipIndex]); // Use clientShips for the client
    }
    ...
    if (isCursorOnShip(cursorX, cursorY, serverShips, 3)) { // Replace 'clientShips' with 'serverShips' for the server code
...
void setup() {
  ...
  initializeServerShips(); // For server side
  initializeClientShips(); // For client side

  display.setCursor(0,0);
  display.print("Hotspot actif");
...
void handleWaitingState() {
  ...
  if (!client || !client.connected()) {
    client = server.available(); // Check for new client connection
    display.print("SSID: ");
    display.println(ssid);
    display.print("Pass: ");
    ...
    display.print("Waiting for a client");
  } else {
    ...
    if (!serverReady) {
      ...
      if (keypad.getKey() == 'Y') {
        serverReady = true;
        client.print("start"); // Send start message to client
      }
    } else if (!clientReady) {
      display.setCursor(0, 30);
...
void handlePlayingState() {
  if (client.available() > 0) {
      ...
      if (message == "isServerTurn=true") {
          isServerTurn = true;
          //Serial.println("server turn");
      }
  ...
  display.clearDisplay();
  if (areAllShipsSunk(serverShips)) {
      client.print("Lose");
      loss = true;
      gameState = RESULTING;
      }
